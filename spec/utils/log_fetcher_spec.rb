...
describe LogFetcher, :broken_in_travis => true do
  ...
  describe "success" do
    ...
    it "should retrieve a log over http" do
      LogFetcher.new(@attr).fetch.should include('<?xml version="1.0"?>')
    end
  end
  ...
  describe "failure" do
    ...
    it "should error if svn doesn't exit with 0" do
      lambda do
        LogFetcher.new(@attr.merge(:repository_url => 'localhost')).fetch
      end.should raise_error(Errors::SvnError)
    end
  end
  ...
  describe "password" do
    ...
    it "should return a password parameter given a password" do
      http_log = LogFetcher.new(@attr.merge(:password => "password"))
  ...
  describe "revisions" do
    ...
    it "should return a range if given a from and to" do
      http_log = LogFetcher.new(@attr.merge(:revision_to => '2'))
    ...
    it "should return a range if given a from of HEAD but no to" do
      http_log = LogFetcher.new(@attr.merge(:revision_from => 'HEAD'))
      http_log.revisions.should eq '-rHEAD:0'
    end

    it "should return blank if given no from or to" do
      http_log = LogFetcher.new(@attr.merge(:revision_from => ''))
